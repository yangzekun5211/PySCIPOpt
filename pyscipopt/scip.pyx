...
cdef class Model:
    ...
    #TODO should we actually support this? or should the user get around with getVars and getNBinVars, etc?
    def getVarsData(self):
        """Retrieve all variables along with the numbers of different variable types:
            binaries, integers, implied integers and continuous
        """
        cdef SCIP_VAR** vars
        cdef int nvars
        cdef int nbinvars
        ...
        PY_SCIP_CALL(SCIPgetVarsData(self._scip, &vars, &nvars, &nbinvars, &nintvars, &nimplvars, &ncontvars))
        return [Variable.create(vars[i]) for i in range(nvars)], nbinvars, nintvars, nimplvars, ncontvars
    ...
    def includeSepa(self, Sepa sepa, name, desc, priority, freq, maxbounddist = 1.0, usessubscip=False, delay=False):
        """Include a separator
    ...
    # Parameter Methods
    # TODO: Is there a way to mimic the valueptr functionality?
    ...
    def addBoolParam(self, name, desc, defaultvalue, isadvanced = False):
        ...
        n = str_conversion(name)
        d = str_conversion(desc)
        PY_SCIP_CALL(SCIPaddBoolParam(self._scip, n, d, NULL, isadvanced, defaultvalue, NULL, NULL))
